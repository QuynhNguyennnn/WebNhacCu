...
public class DAO {
    ...
    public void Pay(String id_Peo, float total) {
        String query = "DELETE [Cart] WHERE [Customer_ID] = ?\n"
                ...
                + "else if (SELECT [Accumulated_points] FROM [Customer] WHERE [Customer_ID] = ?) >= 10000000\n"
    ...
    public List<Liquidation> getAllLiquidation() {
        ...
        String query = "select * from Liquidation";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
    ...
    public List<Liquidation> getWatingLiquidation() {
        List<Liquidation> list = new ArrayList<>();
        String query = "select * from Liquidation where SellStatus = 0";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(query);
            rs = ps.executeQuery();
    ...
    public void setLiquidationProduct(String title, String description, String price, String image, String contactDescrip, int ID_cus, String cate_id) {
        String query = "insert into Liquidation\n"
                + "values (?, ?, ?, ?, ? , 0, ?, GETDATE(), ?)";
        try {
            // Open connection with SQL Server
            conn = new DBContext().getConnection();
            ...
            ps.setString(4, contactDescrip);
            ps.setString(5, price);
            ps.setInt(6, ID_cus);
            ps.setString(7, cate_id);
    ...
    public List<Liquidation> getLiquidationByID(String ID) {
        List<Liquidation> list = new ArrayList<>();
        String query = "select * from Liquidation where ID = ?";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(query);
            ps.setString(1, ID);
    ...
    public List<Liquidation> getLiquidationByCusID(int ID_cus) {
        List<Liquidation> list = new ArrayList<>();
        String query = "select * from Liquidation where Cus_ID = ?";
        try {
            conn = new DBContext().getConnection();
            ps = conn.prepareStatement(query);
            ps.setInt(1, ID_cus);
    ...
    public void deleteLiquidation(String pid) {
        String query = "delete from Liquidation\n"
                + "where ID = ?";
        try {
            // Open connection with SQL Server
            conn = new DBContext().getConnection();
    ...
    public void editLiquidationStatus(String pid) {
        String query = "update Liquidation \n"
                + "set SellStatus = 1\n"
                + "where ID = ?";
        try {
            // Open connection with SQL Server
